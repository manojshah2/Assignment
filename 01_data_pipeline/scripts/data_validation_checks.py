...
def raw_data_schema_check():
    ...
    lead_data = pd.read_csv(DATA_DIRECTORY + LEAD_SCORING_DATA, index_col=[0])
    check = set(lead_data.columns) == set(raw_data_schema)
    if check:
...
def model_input_schema_check():
    ...
    model_input_data = pd.read_sql("select * from model_input", con=con)

    check = set(model_input_data.columns) == set(model_input_schema)
